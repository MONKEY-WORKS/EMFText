...
public class SourceViewerConfigurationGenerator extends UIJavaBaseGenerator<ArtifactParameter<GenerationContext>> {
	...
	private void addGetPresentationReconcilerMethod(JavaComposite sc) {
		...
		sc.add(damagerRepairerClassName + " damagerRepairer = new " + damagerRepairerClassName + "(getScanner(), resourceProvider);");
		sc.add("reconciler.setDamager(damagerRepairer, " + I_DOCUMENT(sc) + ".DEFAULT_CONTENT_TYPE);");
		sc.add("reconciler.setRepairer(damagerRepairer, " + I_DOCUMENT(sc) + ".DEFAULT_CONTENT_TYPE);");
		sc.addLineBreak();
		sc.add("return reconciler;");
		sc.add("}");
		sc.addLineBreak();
